...
class ToastNotifier(object):
    ...
    def show_toast(self, title="Notification", msg="Here comes the message",
                    icon_path=None, duration=5, threaded=False, dbm=False):
        """Notification settings.
        ...
        :icon_path: path to the .ico file to custom notification
        :duration: delay in seconds before notification self-destruction
        :dbm: destroy by myself
        """
        if dbm:
            self.custom_show_toast(title, msg, icon_path, duration)
            return True
        else:
            if not threaded:
    ...
    def custom_show_toast(self, title, msg,
                    icon_path, duration):
        """Notification settings.
    ...
    def custom_destroy(self):
        """
        2020/10/22
        新增custome_destroy，使toaster在自己需要的时候destroy掉
        """
